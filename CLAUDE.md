...
> 🔴 2026-08-03: consolidação do motor de mensagens — Etapa 1 (ADR-020, PR #325). Migration `20260802120000_motor_mensagens_fundacao` **precisa ser aplicada ANTES do merge**, invertendo o hábito do projeto: o `GET /me` do portal passou a ler `marketingOptOut`, e subir o código antes da coluna existir dá **P2022** e quebra a tela de perfil de todo cliente de todo tenant. Migration aditiva com código antigo rodando é segura; o inverso não é. `npx prisma migrate deploy` na porta **5432**, sem backfill. Ela carrega também as colunas das Etapas 2 e 3 (uma janela em vez de três).
