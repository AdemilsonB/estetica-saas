...
> ⚠️ 2026-07-26: motor de mensagens ao cliente (ADR-017) — rodar em produção **na mesma janela e nesta ordem**: `npx prisma migrate deploy` (migration `20260726120000_add_customer_message_template`) e **logo em seguida** `npm run messages:backfill` (aceita `--dry-run` para conferir antes). Sem o backfill, todo tenant que já customizou `mensagemPrincipal`/`mensagemFinal` volta a receber o texto padrão do sistema — não quebra nada, mas perde a personalização dele. ⚠️ A migration foi **gerada offline** (`prisma migrate diff`, banco local indisponível na sessão) e **nunca rodou contra um Postgres real** — validar antes do merge.
