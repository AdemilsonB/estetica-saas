...
> ⚠️ 2026-07-31: mensagem agendada um-a-um (ADR-019). Migration `20260731120000_add_scheduled_message` **pendente de aplicação manual** em produção: `npx prisma migrate deploy` na porta **5432**. **Não há backfill** — a tabela nasce vazia e nenhum comportamento existente muda. A migration foi escrita à mão e conferida com `prisma migrate diff --from-config-datasource` contra o schema real de produção (bateu exatamente, ver ADR-019) — mas ainda não foi *aplicada*; isso é o `npx prisma migrate deploy` da janela de produção, pendente.
> ✅ 2026-08-03: consolidação do motor de mensagens — Etapa 1 (ADR-020, PR #325). Migration `20260802120000_motor_mensagens_fundacao` **aplicada em produção** via `npx prisma migrate deploy` (porta 5432; `prisma migrate status` confirma schema em dia). **Incidente real:** o código (PR #325) foi mergeado/deployado em produção antes da migration rodar, exatamente o cenário que este aviso tentava evitar — `GET /api/scheduling/services` e `/api/crm/customers` caíram com 500 (`P2022`, coluna `Service.returnIntervalDays` inexistente) até a correção. Sem backfill, como previsto — a tabela/colunas nascem vazias e nenhum comportamento muda além de resolver o erro. **Lição:** o runbook "aplicar migration antes do merge" precisa de um passo de verificação automatizado ou humano no processo de deploy — hoje depende de lembrança manual.
> ⚠️ 2026-08-08: pendência de conclusão de atendimento + observação editável (ADR-022). Migration `20260808195825_add_appointment_completion_pending` **pendente de aplicação manual** em produção: `npx prisma migrate deploy` na porta **5432**. Aditiva — `Appointment.completionSnoozedUntil` (nullable) e `SchedulingPolicy.pendingCompletionGraceHours` (default 24). **Sem backfill**, mas com a lição do ADR-020: aplicar a migration **antes** de mergear/deployar o código, não depois — o código novo lê `pendingCompletionGraceHours` em toda leitura de política de agendamento (`GET /api/scheduling/policy`), e sem a coluna isso quebraria com P2022 exatamente como aconteceu na Etapa 1 do motor de mensagens.

---

## Testes vermelhos conhecidos na `main` (2026-08-03)

Três testes falham por causas anteriores à PR #325. **Não são flaky e não devem ser "ajustados para passar"** — cada um documenta um problema real:

| Teste | O que revela |
|---|---|
| `scheduling.service.update` › markPayment atomicidade | `netAmount` vira `NaN`. É o item crítico "checkout não-atômico" da auditoria de QA de 2026-06-27 |
| `customer-history-client` (2 testes) | O bloco "Informações" do Portal aparece quando há endereço mas **nunca renderiza o endereço** — só o horário. Tenant com endereço e sem horário vê um card vazio. E o card de localização com mapa Google + "Ver no Google", que este arquivo documenta como existente no Portal, **não existe no componente** |
