...
> ✅ 2026-08-08: pendência de conclusão de atendimento + observação editável (ADR-022). Migration `20260808195825_add_appointment_completion_pending` **aplicada em produção** via `npx prisma migrate deploy` (porta 5432; `prisma migrate status` confirma schema em dia) — **antes** do merge da PR, seguindo a lição do ADR-020. Aditiva — `Appointment.completionSnoozedUntil` (nullable) e `SchedulingPolicy.pendingCompletionGraceHours` (default 24). Sem backfill, nenhum comportamento existente mudou.
