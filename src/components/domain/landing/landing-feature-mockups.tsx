...
function MockShell({ children, label }: { children: React.ReactNode; label: string }) {
  return (
    <div className="w-full overflow-hidden rounded-none border border-[#EDE1D1] bg-white shadow-xl">
...
export function MockAgenda() {
  const slots = [
    { time: '09h', label: 'Ana · Escova', color: 'bg-[#F3E6D4] text-[#7A4227]', span: 'row-span-1' },
    { time: '10h', label: 'livre', color: 'bg-slate-50 text-slate-300', span: '' },
    { time: '11h', label: 'Bia · Coloração', color: 'bg-[#EDE1D1] text-[#7A4227]', span: '' },
  ...
  return (
    <MockShell label="agend.com.br/agenda">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="rounded-full bg-[#F3E6D4] px-2 py-0.5 text-[10px] font-semibold text-[#B9673C]">
          Mariana
        </span>
      </div>
      ...
      <div className="mt-3 rounded-lg border border-dashed border-[#EDE1D1] bg-[#FBF4EA] px-3 py-2 text-center text-[11px] font-medium text-[#B9673C]">
      ...
      <div className="space-y-3">
        {messages.map((m, i) => (
          <div key={m.tag} className="relative pl-5">
            {i < messages.length - 1 && (
              <span className="absolute left-[7px] top-5 h-full w-px bg-[#EDE1D1]" />
            )}
            <span className="absolute left-0 top-1.5 h-3.5 w-3.5 rounded-full border-2 border-[#B9673C] bg-white" />
            <div className="flex items-center gap-2">
              <span className="rounded bg-[#F3E6D4] px-1.5 py-0.5 text-[10px] font-semibold text-[#B9673C]">
                {m.tag}
              </span>
              <span className="text-[10px] text-slate-400">{m.when}</span>
            </div>
            <div className="mt-1 rounded-lg rounded-tl-none bg-green-50 px-3 py-2 text-xs leading-snug text-green-900">
              {m.text}
            </div>
          </div>
        ))}
  ...
  return (
    <MockShell label="agend.com.br/financeiro">
      ...
      <div className="flex h-20 items-end gap-1.5">
        {bars.map((h, i) => (
          <div key={i} className="flex-1 rounded-t bg-[#B9673C]" style={{ height: `${h}%` }} />
        ))}
      </div>
  ...
  return (
    <MockShell label="perfil da cliente">
      <div className="flex items-center gap-3">
        <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#B9673C] text-base font-bold text-white">
          A
        </div>
      ...
      <div className="mt-3 rounded-lg bg-[#F3E6D4] px-3 py-2 text-xs font-medium text-[#7A4227]">
      ...
      <div className="mt-3 space-y-1.5">
        {['Escova + Hidratação · 12 jun', 'Coloração completa · 28 mai', 'Corte + Escova · 05 mai'].map((h) => (
          <div key={h} className="flex items-center gap-2 text-xs text-slate-500">
            <span className="h-1.5 w-1.5 rounded-full bg-[#EDE1D1]" />
            {h}
          </div>
        ))}
      </div>
