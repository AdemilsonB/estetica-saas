...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existing = await prisma.catalogProductCategory.findUnique({ where: { slug: input.slug } })
    if (existing) {
      return Response.json({ error: { message: 'Slug já em uso' } }, { status: 409 })
    }

    const category = await prisma.catalogProductCategory.create({ data: input })
