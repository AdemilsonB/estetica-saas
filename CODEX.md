...
| Notifications | ✅ | ✅ | Evolution API primário (WhatsApp), 6 templates; **central de notificações da equipe** (PR #277 backend + #278 UI) — dispatcher genérico orientado a `eventType` (substitui `notifyAppointment`), e-mail durável via fila `pg-boss`/`tick`, resolvedor de canais (quiet hours + modo digest anti-fadiga, ambos fuso-aware), motor de templates com fallback do sistema, resumo diário (08:00 fuso do tenant); aba Configurações › Notificações com editor de mensagem + matriz evento×canal + preferências do colaborador, painel de 3 switches removido |
