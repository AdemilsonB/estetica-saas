...
| Scheduling | ✅ | ✅ | Agenda semanal, slots configuráveis por tenant; disponibilidade pública respeita `minAdvanceMinutes`/`maxAdvanceDays` da policy (painel não — só bloqueia data passada, com switch "lançar atendimento esquecido" — ADR-014); picker de agendamento unifica visual de Serviço/Pacote/Promoção (card padronizado com composição/duração + ícone de olho para detalhe completo via `PickerDetailModal`), "Todos" mostra só Serviços, Pacotes/Promoções só ao filtrar; corrigido bug de `promotionId` que ficava aplicado após trocar para um Serviço avulso na vitrine pública, e o bug antigo de desconto perdido em promoção de serviço avulso agendada pela vitrine pública |
