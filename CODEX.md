...
| Notifications | ✅ | ✅ | Evolution API primário (WhatsApp), 6 templates; **central de notificações da equipe** — dispatcher genérico orientado a `eventType` (substitui `notifyAppointment`), e-mail durável via fila `pg-boss`/`tick`, resolvedor de canais (quiet hours + modo digest anti-fadiga, ambos fuso-aware), motor de templates com fallback do sistema, resumo diário; UI (aba Configurações) é entrega separada (PR #277) |
