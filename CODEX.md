...
| Scheduling | ✅ | ✅ | Agenda semanal, slots configuráveis por tenant; disponibilidade pública respeita `minAdvanceMinutes`/`maxAdvanceDays` da policy (painel não — só bloqueia data passada, com switch "lançar atendimento esquecido" — ADR-014); picker de agendamento unifica visual de Serviço/Pacote/Promoção (card padronizado com composição/duração + ícone de olho para detalhe completo via `PickerDetailModal`), "Todos" mostra só Serviços, Pacotes/Promoções só ao filtrar; corrigido bug de `promotionId` que ficava aplicado após trocar para um Serviço avulso na vitrine pública, e o bug antigo de desconto perdido em promoção de serviço avulso agendada pela vitrine pública; **timeline clicável da Visão Dia (2026-07-30)** — grade de slots de 30 em 30 min (vazios inclusive) dentro do horário de expediente do tenant, `AgendaDayTimeline`, 1 coluna quando 0/1 profissional filtrado, N colunas quando N; clicar em slot vazio pré-preenche `CreateAppointmentModal` (data/horário/profissional); profissional agora sempre tem default (usuário logado, ou o vinculado ao serviço escolhido); "+`" pré-preenche a data com a selecionada na agenda; endpoint novo `GET /api/scheduling/business-hours` (gate `agenda:view`, não `configuracoes:view`); `POST /api/scheduling/appointments` passou a rejeitar `professionalId` de outro profissional sem `agenda:edit` (antes só a UI escondia o campo); **refinamentos da timeline (2026-07-30, PR #312):** separador vertical entre colunas de profissionais; card usa CSS grid (`grid-row` via `appointmentSlotSpan()`) pra cobrir a duração real e bloquear clique só na coluna daquele profissional (antes o card só ocupava a altura do próprio conteúdo, deixando os slots seguintes clicáveis por baixo); store `useAgendaDateStore` unifica a data selecionada entre o "+" do header e o FAB da bottom nav global (que antes sempre abria em "hoje"); campo Data do modal virou Popover+Calendar com aplicação imediata; layout Data/Horário foi de 75/25 para 60/40; **pendência de conclusão + observação editável (ADR-022, 2026-08-08):** `Appointment.notes` ganhou UI (já existia no schema) — editável a qualquer momento no `CreateAppointmentModal`/`AppointmentDrawer`, mesmo pós-conclusão/cancelamento, via `PATCH /api/scheduling/appointments/[id]`; "pendente" = SCHEDULED/CONFIRMED com `endsAt` além de `SchedulingPolicy.pendingCompletionGraceHours` (default 24h, configurável), calculado on-the-fly sem job novo; card no Dashboard + badge "Vencido" na timeline; `POST /api/scheduling/appointments/[id]/snooze` (1/3/7 dias) grava `completionSnoozedUntil` sem mudar status; escopo por `agenda:view_all` |
| Financial | ✅ | ✅ | Checkout, despesas, comissões, taxas, estornos |
| Notifications | ✅ | ✅ | Evolution API primário (WhatsApp); **motor de mensagens ao cliente (ADR-017 texto + ADR-018 disparo)** — catálogo em código (`customer-message-catalog.ts`, 10 eventos × 2 canais) é a fonte única do texto padrão, `CustomerMessageTemplate` guarda só a personalização do tenant (ausência = usa padrão); `CustomerMessageSetting` guarda o liga/desliga por evento (7 transacionais ligados, 3 promocionais desligados por padrão), `customerMessageDispatcherService` é o único caminho de envio ao cliente, `<CustomerMessageToggle>` plugado em criar/cancelar/confirmar/remarcar/no-show, `notify?: boolean` cru no evento resolvido só em notifications, `Appointment.origin` discrimina pedido online (`appointment_requested`) de painel (`appointment_created`); **central de notificações da equipe** (PR #277 backend + #278 UI) — dispatcher genérico orientado a `eventType`, e-mail durável via fila `pg-boss`/`tick`, resolvedor de canais (quiet hours + digest, fuso-aware), motor de templates com fallback do sistema, resumo diário (08:00 fuso do tenant); aba Configurações › Notificações reorganizada (PR #305, 2026-07-27) — 2 abas ("Notificações da equipe" fundindo avisos+preferências pessoais, e "Mensagens ao cliente"), deep-link por `?tab=`, cards agrupados em `/configuracoes` sob "Notificações e mensagens" |
